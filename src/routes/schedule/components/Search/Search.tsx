...
export default function Search({ classes, setLoading, viewData }: Props) {
  ...
  return (
    <section className="md:col-span-5 md:row-span-6 bg-c1 rounded-lg box-border flex flex-col max-md:order-2">
      ...
      {current === "search" && deferredCurrent === current ? (
        <>
          <Suspense fallback={<ClassesLoader />}>
            <label className="relative md:mx-4 mx-2 mt-2">
              ...
              {searchInfo && (
                <div className="absolute z-20 top-0 right-0 md:translate-x-full -translate-x-10 bg-[white] p-2 rounded-lg md:max-w-xs w-[70dvw]">
                  <p>
                    You can search any keyword for the class you are looking
                    for.
                  </p>
                  <br />
                  <p>
                  ...
                  <ul className="pl-4">
                    <li className="list-disc">
                      <span className="font-bold">r{">"}4.5</span> (teachers
                      with +4.5/5 rating. Symbol can be {"<"} , {">"} or {"="} )
                    </li>
                    <li className="list-disc">
                    ...
                    <li className="list-disc">
                      <span className="font-bold">honours</span> (honours
                      classes. Special keywords include "honours" and "blended")
                    </li>
                    <li className="list-disc">
                      <span className="font-bold">
                        p=Steven Randall, blended
                      </span>{" "}
            ...
            <div className="w-full h-full box-border rounded-lg overflow-y-auto md:p-4 p-2">
              {deferredInput === input ? (
                <Classes
                  input={deferredInput}
                  classes={classes}
      ...
      {current === "saved" && <ChosenCourses viewData={viewData} />}
    </section>
  );
}
